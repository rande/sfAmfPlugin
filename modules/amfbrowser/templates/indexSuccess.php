...
<head>
<?php use_stylesheet('/sfAmfPlugin/css/sfAmfPluginBrowser.css') ?>
<?php use_javascript('/sfAmfPlugin/js/mootools-core.js') ?>
<?php use_javascript('/sfAmfPlugin/js/mootools-more.js') ?>
<?php use_javascript('/sfAmfPlugin/js/sfAmfPluginBrowser.js') ?>
  ...
  <div class="methods">
    ...
    <?php foreach ($services_reflections as $package_name => $class): ?>
      <div class="inner service<?php echo $class->getName() == $service_name ? ' active' : ' is_fold' ?>">
        ...
        <?php if (!$method->isPublic() or $method->getDeclaringClass()->getParentClass() == null) continue; ?>

        <?php $is_empty = false; ?>
          <li
            class="method"><?php echo link_to($method->getName(), 'amfbrowser/index?method=' . urlencode($package_name . '::' . $method->getName()));?></li>
        <?php endforeach;?>
